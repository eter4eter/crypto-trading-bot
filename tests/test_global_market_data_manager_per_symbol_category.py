...
class TestGlobalMarketDataManagerPerSymbolCategory(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_register_creates_keys_with_categories(self) -> None:
    ...
    async def test_ws_subscribe_uses_category(self) -> None:
        self.manager.register_strategy(self.strategy_cfg, self.callback)
        calls = []
    ...
    async def test_polling_grouped_by_frame_and_category(self) -> None:
        self.manager.register_strategy(self.strategy_cfg, self.callback)
    ...
    async def test_distribute_polling_respects_category(self) -> None:
        frame = "5s"
        category = "spot"
        subs = [
